...
int get_arg(char *argv[])
{
	...
	while (getline(&line, &size, fp) != -1)
	{
		global.line = line;
		count++;
		get_buffer(line, buffer); /* to add token to the line*/
		if (buffer != NULL)
		{
...
/**
 * exe - selects the operation and executes them
 * @op: operation
 * @stack: stack root
...
/**
 * get_buffer - takes arguments and saves info in buffer
